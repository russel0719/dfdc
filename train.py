...
def train_epoch(current_epoch, loss_functions, model, optimizer, scheduler, train_data_loader, summary_writer, conf,
    ...
    for i, sample in pbar:
        ...
        acc_labels = [1 if label > 0.5 else 0 for label in labels]
        acc_preds = [1 if pred > 0.5 else 0 for pred in out_labels]
        count = sum([1 if l == p else 0 for l, p in zip(acc_labels, acc_preds)])
        ...
        wandb.log({
            "Train loss": loss / imgs.size(0),
            "Train accuracy": 100 * (count / len(acc_labels))
        })
        loss.backward()
        optimizer.step()
